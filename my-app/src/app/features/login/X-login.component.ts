...
export class LoginComponent implements OnInit {
  ...
  onGetUser(userEmail: string, password: string) {
    ...
    // subscription.unsubscribe(); // We CANNOT unsubscribe here. App stop listening before passing the result to us.
    // push this observable to the array. We'll unsubscribe onDestroy.
    this.subscriptions.push(subscription); // no need. see below.
  }
  ...
  onLogin(userEmail: string, password: string) {
    ...
    // Http request closed once it's done, no need to push into subscriptions or unsubscribe
    // // but we'll celan up anyway.
    // subscription.unsubscribe(); // we CANNOT unsubscribe HERE!
